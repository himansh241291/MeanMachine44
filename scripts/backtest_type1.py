...
def backtest(daily: pd.DataFrame, variant: str, horizon: int = 20, lookback: int = 5) -> list[dict]:
    ...
    for i, row in daily.iterrows():
        ...
        trigger = next((j for j in range(i + 1, end) if daily.iloc[j]["high"] > candle.high), None)
        if trigger is None:
            continue
        entry = candle.high
        stop = candle.low
        ...
        for multiple in TARGETS:
            outcome = first_touch(future_highs, future_lows, target_levels[f"{multiple}R"], stop)
